...
class Detect :
    ...
    def __init__(self) -> None:
        ...
        rospy.Subscriber('/odom_frenet', Odometry, self.carStateCb)
        ...
        # --- dyn params sub ---
        self.min_obs_size = 0.2
        self.max_obs_size = 0.55
        self.max_viewing_distance = 7.5
        self.boundaries_inflation = 0.25
        ...
        try:
            rospy.loginfo("[Opponent Detection]: Waiting for global path topic...")
            # 10초 타임아웃으로 /global_path/optimal_trajectory_wpnt 토픽을 기다림
            initial_wpnts_msg = rospy.wait_for_message("/global_path/optimal_trajectory_wpnt", WpntArray, timeout=10.0)
            self.pathCb(initial_wpnts_msg) # 받은 메시지로 콜백 함수를 직접 호출
            rospy.loginfo("[Opponent Detection]: Global path received.")
            self.converter = self.initialize_converter()
        except rospy.ROSException:
            rospy.logerr("[Opponent Detection]: Timeout waiting for global path topic. Is lane_visualize.py running and publishing?")
            rospy.signal_shutdown("Required topic not available.")
            return # 노드 종료
    ...
    def calculate_frenet(self, x, y, waypoints):
        """
        Calculate Frenet coordinates (s, d) for a given obstacle position (x, y),
        using waypoints with heading information.
        
        Args:
            x (float): Obstacle x-coordinate.
            y (float): Obstacle y-coordinate.
            waypoints (np.ndarray): Array of waypoints, where each waypoint is [x_m, y_m, s_m, psi_rad].
            
        Returns:
            (float, float): Frenet coordinates (s, d).
        """
        wp_x = waypoints[:, 0]
        wp_y = waypoints[:, 1]
        wp_s = waypoints[:, 2]
        ...
        # Step 1: Find the closest waypoint
        distances = np.sqrt((wp_x - x)**2 + (wp_y - y)**2)
        closest_idx = np.argmin(distances)

        # Step 2: Calculate s (longitudinal distance)
        s = wp_s[closest_idx]

        # Step 3: Calculate d (lateral distance)
        # Heading vector of the closest waypoint
        psi = wp_psi[closest_idx] * np.pi / 180
        heading_vector = np.array([np.cos(psi), np.sin(psi)])
        
        # Vector from the closest waypoint to the obstacle
        position_vector = np.array([x - wp_x[closest_idx], y - wp_y[closest_idx]])

        # Perpendicular vector to the heading
        perp_vector = np.array([-heading_vector[1], heading_vector[0]])  # Rotate 90 degrees

        # Calculate d as the projection of the position vector onto the perpendicular vector
        d = np.dot(position_vector, perp_vector)
    ...
    def pathCb(self, data):
        ...
        self.waypoints = np.array([[wpnt.x_m, wpnt.y_m,wpnt.s_m,wpnt.psi_rad] for wpnt in data.wpnts])
        ...
        if (self.s_array is None or self.path_needs_update) and self.converter is not None:
            ...
            for waypoint in waypoint_array:
                ...
                self.d_right_array.append(self.d_right-self.boundaries_inflation)
                self.d_left_array.append(self.d_left-self.boundaries_inflation)
                resp = self.converter.get_cartesian(waypoint.s_m,self.d_right+self.boundaries_inflation)
                points.append(Point(resp[0],resp[1],0))
                resp = self.converter.get_cartesian(waypoint.s_m,self.d_left-self.boundaries_inflation)
                points.append(Point(resp[0],resp[1],0))
    ...
    def carStateCb(self, data):
        self.car_s, _ = data.pose.pose.position.x, data.pose.pose.position.y
    
    def dyn_param_cb(self, params: Config):
        self.min_obs_size = rospy.get_param('dynamic_tracker_server/min_obs_size', 11)
        self.max_obs_size = rospy.get_param('dynamic_tracker_server/max_obs_size', 0.55)
        self.max_viewing_distance = rospy.get_param('dynamic_tracker_server/max_viewing_distance', 8)
        self.boundaries_inflation = rospy.get_param('dynamic_tracker_server/boundaries_inflation', 0.09)
    ...
    def scans2ObsPointCloud(self):
        ...
        try:
            trans, quat = self.tf_listener.lookupTransform('/map', '/laser', rospy.Time(0))
        except:
            rospy.logerr(f"[Opponent Detection]: TF transform error between map and laser: {e}")
            empty = []
            return empty
        ...
        for obstacle in objects_pointcloud_list:
            x_points.append(obstacle[int(len(obstacle)/2)][0])
            y_points.append(obstacle[int(len(obstacle)/2)][1])

        # # Calculate Frenet coordinates
        s_points, d_points = [], []
        s_points, d_points = self.converter.get_frenet(np.array(x_points),np.array(y_points))# self.converter.get_frenet(x_center, y_center)
    ...
    def publishObstaclesMessage(self):
        ...
        # # Calculate Frenet coordinates
        s_points, d_points = [], []
        s_points, d_points = self.converter.get_frenet(x_center,y_center)# self.converter.get_frenet(x_center, y_center)
        # for x, y in zip(x_center, y_center):
        #     s, d = self.calculate_frenet(x, y, self.waypoints)# self.converter.get_frenet(x_center, y_center)
        #     s_points.append(s)
        #     d_points.append(d)

        # Create obstacle messages
        for idx, obstacle in enumerate(self.tracked_obstacles):
            ...
            obstacles_array_message.obstacles.append(obs_msg)
