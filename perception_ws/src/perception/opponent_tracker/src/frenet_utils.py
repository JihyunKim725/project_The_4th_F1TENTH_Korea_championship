...
class FrenetConverter:
    def __init__(self, waypoints_x: np.array, waypoints_y: np.array, waypoints_s: np.array):
        self.waypoints_x = waypoints_x
        self.waypoints_y = waypoints_y
        self.waypoints_s = waypoints_s
        self.spline_x = None
        self.spline_y = None
    ...
    def get_approx_s(self, x, y) -> np.ndarray:
        """
        Finds the s-coordinates of the given points by finding the nearest waypoint for each point.
        ...
        Returns:
            np.ndarray: Corresponding s-coordinates for the given points.
        """
        x = np.array(x, ndmin=1)
        y = np.array(y, ndmin=1)

        # Calculate distances from each input point to all waypoints
        lenx = len(x)
        dist_x = x - np.tile(self.waypoints_x, (lenx, 1)).T
        dist_y = y - np.tile(self.waypoints_y, (lenx, 1)).T
        distances = np.linalg.norm([dist_x.T, dist_y.T], axis=0)

        # Find the index of the nearest waypoint for each point
        nearest_idx = np.argmin(distances, axis=1)

        # Ensure waypoints_s is a NumPy array
        self.waypoints_s = np.array(self.waypoints_s, dtype=float)

        # Return the corresponding s_m values
        return self.waypoints_s[nearest_idx]
